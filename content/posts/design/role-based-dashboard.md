...
highlights:
  - id: C-11
    before: "8개 역할이 있고, 역할마다 볼 수 있는"
    after: "8개 역할이 있고 역할마다 볼 수 있는"
  - id: C-11
    before: "유지보수가 불가능하고, 하나의 대시보드에서"
    after: "유지보수가 불가능하고 하나의 대시보드에서"
  - id: C-11
    before: "형태고, 키가 라우트 슬러그에"
    after: "형태고 키가 라우트 슬러그에"
  - id: C-11
    before: "보이지만, 운영상 필요한 예외였다"
    after: "보이지만 운영상 필요한 예외였다"
  - id: C-11
    before: "되지만, 역할-메뉴 매핑이"
    after: "되지만 역할-메뉴 매핑이"
residual_findings: (없음)
grade_reason: "A — S1 0건, 변경률 0.2%, 자체검증 6항 통과. 원문 자체가 개발자 블로그 문체로 깨끗하여 최소한의 교정만 수행. 기술 블로그 register 유지."
-->
